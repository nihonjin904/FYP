...
public:	
	...
	/** 擋刀時用嘅旋轉軸：手 (hand_r) → 此 Pivot → WeaponMesh。擋刀時只轉呢個 Pivot，刀就會打橫，唔會被動畫蓋過。 */
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Sekiro|Weapon")
	TObjectPtr<USceneComponent> BlockWeaponPivot;

	/** 擋刀時套用到武器上的額外旋轉（打橫放等）。留空則用 WeaponMesh。已棄用：建議用 BlockWeaponPivot。 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Sekiro|Weapon", meta = (DisplayName = "Block Weapon Component (Optional Override)"))
	...
	/** 擋刀／完美格擋時播放嘅音效（可在 Blueprint 指定） */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Sekiro|Feedback")
	TObjectPtr<class USoundBase> ParryBlockSound;

	/** 擋刀／完美格擋時嘅黃色閃光粒子（可在 Blueprint 指定） */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Sekiro|Feedback")
	TObjectPtr<class UParticleSystem> ParryBlockParticle;

	/** 擋/彈刀時 Hit Stop 時長（秒），0 = 唔開 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Sekiro|Feedback", meta = (ClampMin = "0", ClampMax = "0.5"))
	float ParryHitStopDuration = 0.06f;

	/** Hit Stop 時嘅時間縮放（愈細愈慢，0.08 = 約 12% 速度） */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Sekiro|Feedback", meta = (ClampMin = "0.01", ClampMax = "1"))
	float ParryHitStopTimeScale = 0.08f;

	/** 擋/彈刀時嘅 Niagara 特效（用 SimpleSpriteBurst 做黃色閃光；Cascade 用 Parry Block Particle） */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Sekiro|Feedback")
	TObjectPtr<class UNiagaraSystem> ParryBlockNiagara;
	...
	/** 鎖定搜尋半徑（公尺） */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Sekiro|LockOn", meta = (ClampMin = "500", ClampMax = "5000"))
	float LockOnRange = 1500.f;

	/** 鎖定時鏡頭轉向目標嘅速度 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Sekiro|LockOn", meta = (ClampMin = "1", ClampMax = "20"))
	float LockOnRotationSpeed = 8.f;

	/** 鎖定時瞄準目標嘅高度偏移（避免鎖地面；數值愈大鏡頭愈抬高） */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Sekiro|LockOn", meta = (ClampMin = "0", ClampMax = "300"))
	float LockOnTargetZOffset = 140.f;

	/** 鎖定時將 Camera Boom 的 Socket Offset Z 抬高（令相機位置唔會太低） */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Sekiro|LockOn", meta = (ClampMin = "-200", ClampMax = "200"))
	float LockOnCameraSocketOffsetZ = 60.f;

	/** 鎖定時 Pitch 夾角限制（防止鏡頭向下） */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Sekiro|LockOn", meta = (ClampMin = "-89", ClampMax = "0"))
	float LockOnPitchMin = -25.f;

	/** 鎖定時 Pitch 夾角限制（防止鏡頭向上過多） */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Sekiro|LockOn", meta = (ClampMin = "0", ClampMax = "89"))
